...
#SET UP OPTIONS...
echo >&2
echo "Running CHIP-seq Pipeline version 1.0.1 started `date`" >&2
echo "Settings:" >&2
echo  >&2
...
mkdir ${RESULTS_DIR}
bowtie --chunkmbs 1024 -y -v 2 -p 7 --best --strata -m 3 -k 1 --sam-nohead \
...
bedtools bamtobed -i ${RESULTS_DIR}/r1_r2_${RUN_NAME}.sam.bam.sorted.filtered.bam \
                   > ${RESULTS_DIR}/r1_r2_${RUN_NAME}.sam.bam.sorted.filtered.bam.bed
echo "done" >&2
echo >&2
wait
...
docker run -v ${RESULTS_DIR}:/data \
    fooliu/macs2 callpeak                           \
              -t /data/r1_r2_${RUN_NAME}.sam.bam.sorted.filtered.bam \
              -n /data/${RUN_NAME}_peaks_macs2 -f BAM -g 773939492   \
              --outdir /data/                       \
...
# cd $ATACBIN
# ./ENCODE_CHIP_PIPELINE_svr4_tn5_shift.sh -n [name of job] -o [path:output directory] \
